...
<body>
  <div class="container">
    ...
    <div id="capture">
      ...
      <section class="card">
        ...
        <div class="grid2">
          ...
          <div class="row"><label for="site" id="siteLabel">점검 장소</label><input id="site" type="text" placeholder="예) ○○현장 타워크레인 #1" /></div>
          <div class="row"><label for="inspector">점검자</label><input id="inspector" type="text" placeholder="성명" /></div>
          <div class="row"><label for="equip">장비 구분</label><input id="equip" type="text" value="타워크레인" /></div>
          <div class="row"><label for="equipId">장비 ID/번호</label><input id="equipId" type="text" placeholder="예) TC-01" /></div>
        </div>
      </section>
      ...
      <section class="card">
        ...
        <div class="hero-box">
          <img id="heroImg" class="hero-img" alt="타워크레인 대표 사진" />
        </div>
      </section>
    ...
    <div class="note" style="margin-top:12px">URL 예: <code>?equipId=TC-01&site=○○현장1동&org=생산1팀&equip=타워크레인&siteLabel=설치위치</code></div>
  </div>
  ...
  <script>
    ...
    const checklistTemplate = [
      {
        key:'foundation_anchor',
        label:'기초 /<br>앵커볼트',
        method:'타워크레인 기초 주변 침하·균열 여부 및 앵커볼트의 풀림, 부식, 손상 여부를 육안으로 확인한다.',
        refImg:'기초_앵커볼트.png'
      },
      {
        key:'mast_section',
        label:'마스트 /<br>접속핀',
        method:'마스트(타워) 및 접속부에 변형·균열·용접부 파손이 없는지 확인하고, 접속핀·핀 고정장치(코터핀 등)가 빠지지 않았는지 확인한다.',
        refImg:'마스트_접속핀.png'
      },
      {
        key:'slewing_ring',
        label:'선회베어링 /<br>고정볼트',
        method:'선회베어링 주변에 과도한 틈새, 이물 끼임, 누유 흔적이 없는지 확인하고, 고정볼트 풀림·파손·누락 여부를 육안으로 확인한다.',
        refImg:'선회베어링.png'
      },
      {
        key:'jib_counterjib',
        label:'지브 /<br>카운터지브',
        method:'지브 및 카운터지브의 처짐, 변형, 균열 여부를 확인하고 보강재, 체결볼트 및 핀의 이탈·풀림이 없는지 확인한다.',
        refImg:'지브_카운터지브.png'
      },
      {
        key:'wire_rope',
        label:'와이어로프',
        method:'와이어로프의 풀림, 꼬임, 가닥 끊김, 과도한 마모·부식 여부를 확인하고 압착 소켓·클립 등의 체결상태를 점검한다.',
        refImg:'와이어로프.png'
      },
      {
        key:'hook_block',
        label:'훅 /<br>훅블록',
        method:'훅 몸체의 균열·변형 유무와 세이프티 래치(훅걸쇠)가 정상적으로 닫히는지 확인하며, 훅블록 회전부 및 연결부 파손·이상 유무를 확인한다.',
        refImg:'훅블록.png'
      },
      {
        key:'safety_devices',
        label:'안전장치<br>(리미트 등)',
        method:'과부하방지장치(LMI), 권과방지장치, 리미트 스위치 등 필수 안전장치의 전원표시와 경보 작동 여부를 시험 또는 육안으로 확인한다.',
        refImg:'안전장치.png'
      },
      {
        key:'ladder_platform',
        label:'승강 사다리 /<br>작업발판',
        method:'사다리 고정상태, 발판 미끄럼 방지 상태, 난간·토보드 설치 상태를 확인하고, 손상·부식·이탈 등 이상이 없는지 확인한다.',
        refImg:'사다리_발판.png'
      },
      {
        key:'electric_panel',
        label:'전기·제어반',
        method:'전기·제어반 문이 닫혀 잠금되어 있는지 확인하고, 내부 과열·변색 흔적, 누전차단기 및 차단기 레버 위치, 케이블 피복 손상 여부를 확인한다.',
        refImg:'전기제어반.png'
      },
      {
        key:'cab_controls',
        label:'운전실 /<br>조종장치',
        method:'조이스틱·페달·비상정지버튼 등의 파손·이탈 여부와 계기판 경고등 이상 유무를 확인하고, 와이퍼·조명 등 기본 기능이 정상 작동하는지 확인한다.',
        refImg:'운전실_조종장치.png'
      },
      {
        key:'surrounding_area',
        label:'주변작업구역 /<br>출입통제',
        method:'회전반경과 부하 이동 경로 내 장애물, 전선·건축물 등 충돌 위험물이 없는지 확인하고, 출입금지 표지판·가설펜스 등 출입통제 상태를 점검한다.',
        refImg:'주변작업구역.png'
      }
    ];
    ...
    function loadParams(){
      ...
      state.org = p.get('org')||'';
      state.site = p.get('site')||'';
      state.inspector = p.get('inspector')||'';
      state.equip = p.get('equip')||'타워크레인';
      state.equipId = p.get('equipId')||'';
      const lbl = p.get('siteLabel');
      if(lbl) $('siteLabel').textContent = lbl;
    }
    ...
    function renderTable(){
      ...
      $('equip').value=state.equip;
      $('equipId').value=state.equipId;

      const hero=$('heroImg');
      hero.src=HERO_SRC;
      hero.onerror=()=>{
        hero.src='data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="800" height="520"><rect width="100%" height="100%" fill="%23f1f5f9"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="sans-serif" font-size="16" fill="%2399a">대표 사진을 불러오지 못했습니다. images/towercrane-hero.png 파일을 확인하세요.</text></svg>';
      };
      ...
      checklistTemplate.forEach((row,i)=>{
        ...
        tr.innerHTML =
          ...
          '<img class="thumb" src="'+(row.refImg||PLACEHOLDER_THUMB)+'" alt="'+row.label.replace(/<br>/g," ")+'" data-full="'+(row.refImg||PLACEHOLDER_THUMB)+'">'+
    ...
    function ensureHtml2Canvas(cb){
      ...
      diag.textContent='이미지 라이브러리 로딩 중...';
      const srcs=[
    ...
    function showBusy(msg){
      ...
      o.querySelector('span').textContent=msg||'처리 중...';
    }
    ...
    function downloadPNG(){
      ensureHtml2Canvas(function(){
        ...
        html2canvas(el,{scale:scale,useCORS:true,backgroundColor:'#ffffff'}).then((canvas)=>{
          ...
          const name='타워크레인_자가진단_'+(($('equipId').value)||'장비')+'_'+(($('date').value)||'').replace(/[:T]/g,'-')+'.png';
